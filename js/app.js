class BingoGen {
    constructor() {
        this.generateButtton = document.getElementById('generateButton')
        this.output = document.getElementById('output')
        this.gameString = 'BINGO';
        this.number;
        this.numberHistory = [];
        this.generateButtton.addEventListener('click', ()=> {
            //console.log('clicked')
            this.randomNumber();
        });
    }
    ...
    update() {
        ...
        for(let i = 1; i < Math.min(6, this.numberHistory.length + 1); i++) {
            this.output.innerHTML +=  `<span> ${this.numberHistory[i]}</span>` 
        }
    }
    ...
    randomNumber() {
        ...
        do {
            ...
            let randomLetter = this.gameString[Math.floor(Math.random() * this.gameString.length)];
              ...
              if (this.numberHistory.length >= 74) {
                  this.numberHistory = [];
              }
    ...
    update() {
        ...
        for(let i in this.nodes) {
            ...
            this.nodes[i].forEach((el) => {
                if(el === 'free') {
                    output += `<div class="col-item red-bg "><span class=" col-text">${el.toUpperCase()}</span></div>`
...
const generator = new BingoGen;

const card = new CardGen;

let hostButton = document.getElementById('displayGenorator');
